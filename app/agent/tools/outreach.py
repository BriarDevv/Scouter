...
def list_drafts(db: Session, *, status: str | None = None, limit: int = 10) -> dict:
...
def reject_draft(db: Session, *, draft_id: str, reason: str | None = None) -> dict:
...
registry.register(
    ToolDefinition(
        name="send_draft",
        description=("Enviar un borrador de outreach aprobado (requiere confirmación)"),
        parameters=[
            ToolParameter("draft_id", "string", "UUID del borrador"),
        ],
        category="outreach",
    ...
    ToolDefinition(
        ...
        parameters=[
            ...
            ToolParameter(
                "subject",
                "string",
                "Nuevo asunto del email",
                required=False,
            ),
            ToolParameter(
                "body",
                "string",
                "Nuevo cuerpo del email",
                required=False,
            ),
        ],
        category="outreach",
...
registry.register(
    ToolDefinition(
        ...
        description=(
            "Listar el historial de actividad de outreach, opcionalmente filtrado por lead"
        ),
        parameters=[
            ToolParameter(
                "lead_id",
                "string",
                "UUID del lead para filtrar",
                required=False,
            ),
            ToolParameter(
                "limit",
