...
def get_lead_dossier(db: Session, *, lead_id: str) -> dict:
    ...
    report = db.query(LeadResearchReport).filter_by(lead_id=uuid.UUID(lead_id)).first()
    if not report:
...
def get_commercial_brief(db: Session, *, lead_id: str) -> dict:
    ...
    return {
        ...
        "contact_priority": (brief.contact_priority.value if brief.contact_priority else None),
    }
