...
def export_leads_csv(db: Session, leads: list[Lead]) -> bytes:
    ...
    writer.writerow([
        "id", "business_name", "industry", "city", "website_url",
        "instagram_url", "email", "phone", "status", "score",
        "llm_quality", "created_at",
    ])
    for lead in leads:
        writer.writerow([
            str(lead.id), lead.business_name, lead.industry, lead.city,
            lead.website_url, lead.instagram_url, lead.email, lead.phone,
            lead.status.value if lead.status else "",
            lead.score, lead.llm_quality,
...
def export_leads_xlsx(db: Session, leads: list[Lead]) -> bytes:
    ...
    ws.title = "Leads"
    headers = [
        "ID", "Negocio", "Industria", "Ciudad", "Website", "Instagram",
        "Email", "Telefono", "Estado", "Score", "Calidad IA", "Creado",
    ]
    ws.append(headers)
    for lead in leads:
        ws.append([
