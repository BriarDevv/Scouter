...
def get_brief_for_lead(
    ...
    if not brief:
        raise HTTPException(
            404, "No commercial brief found for this lead"
        )
    return brief
...
def list_briefs(
    ...
    if budget_tier:
        query = query.filter(
            CommercialBrief.budget_tier == budget_tier
        )
    if contact_priority:
        query = query.filter(
            CommercialBrief.contact_priority == contact_priority
        )
    return (
        query.order_by(CommercialBrief.created_at.desc())
        .limit(limit)
        .all()
    )
